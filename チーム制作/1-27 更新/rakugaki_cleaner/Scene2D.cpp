...
// テクスチャ座標の設定処理
//=============================================================================
void CScene2D::SetTex(D3DXVECTOR2 texUV)
{
	// 構造体のポインタ変数
	VERTEX_2D *pVtx;
	...
	// テクスチャ座標の更新
	pVtx[0].tex = D3DXVECTOR2(texUV.x / m_texSheets.x, texUV.y / m_texSheets.y + 1.0f / m_texSheets.y);
	pVtx[1].tex = D3DXVECTOR2(texUV.x / m_texSheets.x, texUV.y / m_texSheets.y);
	pVtx[2].tex = D3DXVECTOR2(texUV.x / m_texSheets.x + 1.0f / m_texSheets.x, texUV.y / m_texSheets.y + 1.0f / m_texSheets.y);
	pVtx[3].tex = D3DXVECTOR2(texUV.x / m_texSheets.x + 1.0f / m_texSheets.x, texUV.y / m_texSheets.y);

	// 頂点バッファをアンロックする
	m_pVtxBuff->Unlock();
}

void CScene2D::AddPosTex(const D3DXVECTOR3 & pos, const D3DXVECTOR2 & tex, int nNo1, int nNo2)
{
	// ポインタ
	VERTEX_2D *pVtx;
	...
	pVtx[nNo1].pos.x = pos.x;
	pVtx[nNo2].pos.x = pos.x;

	pVtx[nNo1].tex.x = tex.x;
    pVtx[nNo2].tex.x = tex.x;
...
CScene2D * CScene2D::Create(const D3DXVECTOR3 & pos, const D3DXVECTOR3 & size, int nPriority,int nTex)
{
	...
	if (pScene2D == NULL)
	{
		...
		pScene2D->Init(pos, D3DXVECTOR2(size.x, size.y));

		if (nTex >= 0 && pTex != NULL)
		{
			pScene2D->BindTexture(pTex->GetAddress(nTex));
		}
		else if (pTex == NULL)
		{
			pScene2D->BindTexture(NULL);
		}
	}
