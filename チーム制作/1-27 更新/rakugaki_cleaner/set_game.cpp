			...
			if (m_nNumTex < nNumTex)
			{
				...
				if (pTex != NULL)
				{
					...
					if (pBase != NULL)
					{
						...
						if (nNumPlayer > m_nCntEraserStateUI)
						{
							CPlayer_Game **pPlayer = pGame->GetPlayer();

							// テクスチャの生成
							CScene2D *pEraserStateUI = CScene2D::Create(pos, D3DXVECTOR3(size.x, size.y, 0.0f), CScene::PRIORITY_UI, pTex->SetTexture(&pTextureFileName[nTex][0]));
							pEraserStateUI->SetTexSheet(tex);
							pEraserStateUI->SetTex(D3DXVECTOR2(0.0f,1.0f));
							pPlayer[m_nCntEraserStateUI]->SetEraserStateUI(pEraserStateUI);
