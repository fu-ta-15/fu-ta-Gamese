...
// 生成処理
//=============================================================================
CScene3D *CScene3D::Create(D3DXVECTOR3 pos, D3DXVECTOR2 size, D3DXVECTOR3 rot)
{
	// ポインタ変数の生成
	CScene3D *pScene3D;
	pScene3D = new CScene3D(CScene::PRIORITY_FLOOR);

	// NULLチェック
	if (pScene3D != NULL)
	{
		// 角度の指定
		pScene3D->SetRotate(rot);

		// 初期化処理
		pScene3D->Init(pos, size);

		// オブジェクトタイプの設定
		pScene3D->SetObjType(OBJTYPE_FLOOR);
	}
...
HRESULT CScene3D::Init(D3DXVECTOR3 pos, D3DXVECTOR2 size)
{
	...
	// 頂点座標の設定
	pVtx[0].pos = D3DXVECTOR3(0.0f - m_fWidth / 2.0f, 0.0f, 0.0f + m_fDepth / 2.0f);
	pVtx[1].pos = D3DXVECTOR3(0.0f + m_fWidth / 2.0f, 0.0f, 0.0f + m_fDepth / 2.0f);
	pVtx[2].pos = D3DXVECTOR3(0.0f - m_fWidth / 2.0f, 0.0f, 0.0f - m_fDepth / 2.0f);
	pVtx[3].pos = D3DXVECTOR3(0.0f + m_fWidth / 2.0f, 0.0f, 0.0f - m_fDepth / 2.0f);
