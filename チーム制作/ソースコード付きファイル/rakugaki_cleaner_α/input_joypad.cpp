...
// マクロ定義
//*****************************************************************************
#define DEADZONE        (7500)      // 各軸の25%を無効ゾーンとする
...
void CInputJoypad::CrossKeyPressInfo(int nCnt, DIJOYSTATE2 State)
{
	...
	if (State.rgdwPOV[0] >= 21000 && State.rgdwPOV[0] <= 33000)
	{// 右
...
void CInputJoypad::CrossKeyTriggerInfo(int nCnt, DIJOYSTATE2 State)
{
	...
	if (State.rgdwPOV[0] >= 12000 && State.rgdwPOV[0] <= 24000)
	{// 下
...
// ボタン入力情報の取得
//=============================================================================
bool CInputJoypad::GetButtonPress(int nCnt, int nButton)
{// プレス情報の確認
	return (m_Controller[nCnt].State.rgbButtons[nButton] & 0x80) ? true : false;
}

bool CInputJoypad::GetButtonTrigger(int nCnt, int nButton)
{// トリガー情報の確認
	return (m_Controller[nCnt].Trigger.rgbButtons[nButton] & 0x80) ? true : false;
}

bool CInputJoypad::GetButtonReleace(int nCnt, int nButton)
{// リリース情報の確認
