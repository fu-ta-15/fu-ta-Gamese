			...
			if (pScene->GetObjType() == CScene::OBJTYPE_STAGE)
			{
				...
				if (pStage->Collision(&pos, GetSize()))
				{
					...
					if (m_bColStage == false)
					{
						...
						{
							CSkill::Create(pos, D3DXVECTOR2(100.0f + 50.0f * (float)nRank, 100.0f + 50.0f * (float)nRank));
						}
						else
						{
							CSkill::Create(pos, D3DXVECTOR2(200.0f, 200.0f));
						}
